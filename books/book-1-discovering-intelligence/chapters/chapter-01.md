...
第一位保存了一条路线；第二位建立了一张关系图。第一位并不是没有记住，而是不知道哪些信息在条件变化后仍然重要。
...
> **Compression 不等于 Truth。压缩让关系更容易使用和检验，却不能保证关系就是世界的真实机制。**
...
行为、因果和内部机制证据不是三选一，也不能互相替代；它们从不同角度约束解释，只有相互加强时，关于“模型理解了什么”的判断才更可信。
...
本章说明：理解需要关系，而且少量关系可以组织大量经验。

那么，一个复杂世界为什么可以由少量简单关系产生？这些关系怎样组合，最终形成语言、市场、神经网络和智能？
