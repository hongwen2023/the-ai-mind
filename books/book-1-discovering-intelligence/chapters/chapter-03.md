...
错误直觉是：抽象层越高，系统越智能，底层细节越不重要。实际上，LLM API 适合构建应用，却不足以研究训练动态；高级训练框架适合组合模型，却可能隐藏性能瓶颈与数值错误。

更强测试：先写明任务，再比较两个抽象层各自保留和隐藏什么；列出三个必须下钻的触发信号，例如异常、漂移与性能瓶颈。
...
> **抽象不会让现实变简单。它通过隐藏可替换细节、保留任务所需关系，降低我们继续可靠地推理和行动的成本。**

## Bridge to Chapter 4

抽象决定了什么值得保留，却没有规定机器怎样保存它。

“圆形”“高毛利”“风险上升”和“句子含义”都只是人类语言里的名字。人类可以用名字和符号暂存抽象，机器需要更明确的数据结构：数字、类别、像素、Token、向量或其他可计算形式。
